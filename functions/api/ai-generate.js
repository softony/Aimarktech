/* =========================================================
   ...
   Respuestas:
     GET  /api/ai-generate  -> { configured: true|false }   (indicador de estado)
     POST /api/ai-generate  -> { diagnostico: {...} } | { post: {...} } | { configured:false }
...
/* ---------- Generadores ---------- */
async function generateDiagnostico(p, cfg) {
  const raw = await chat(diagnosticoPrompt(p), cfg);
...
async function generatePost(p, cfg) {
  const raw = await chat(postPrompt(p), cfg);
  return parseJSON(raw);
}

/* ---------- Llamada al LLM (OpenAI o Anthropic) ---------- */
async function chat(prompt, cfg) {
  if (cfg.OPENAI_KEY) return chatOpenAI(prompt, cfg);
  if (cfg.ANTHROPIC_KEY) return chatAnthropic(prompt, cfg);
  throw new Error("No API key");
}

async function chatOpenAI(prompt, cfg) {
...
async function chatAnthropic(prompt, cfg) {
