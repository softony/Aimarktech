/* =========================================================
   AIMARKTECH — Cloudflare Pages Function: leads
   ---------------------------------------------------------
   ...
   Cuando configures estas variables de entorno en Cloudflare
   Pages (Settings -> Environment variables), se activa solo:
   ...
   Ruta pública (Cloudflare Pages, file-based routing):
     POST /api/leads          -> inserta un prospecto (público)
     GET  /api/leads?token=   -> lista prospectos (requiere ADMIN_TOKEN)
...
export async function onRequest(context) {
  ...
  if (request.method === "POST") {
    ...
    let lead;
    try {
      lead = JSON.parse((await request.text()) || "{}");
    } catch (e) {
